...
TEST(LockManagerTest, DISABLED_RowAbortTest1) {
...
TEST(LockManagerTest, DISABLED_CompatibilityTest1) {
  ...
  std::thread t0([&]() {
    ...
    EXPECT_TRUE(res);
    std::this_thread::sleep_for(std::chrono::milliseconds(50));
    LOG_INFO("# t0 check txn1");
    CheckTableLockSizes(txn1, 0, 0, 0, 0, 0);
    LOG_INFO("# t0 check txn2");
    CheckTableLockSizes(txn2, 0, 0, 0, 0, 0);
...
TEST(LockManagerTest, DISABLED_CompatibilityTest3) {
  LockManager lock_mgr{};
  TransactionManager txn_mgr{&lock_mgr};
  table_oid_t oid = 0;
...
TEST(LockManagerTest, DISABLED_AbortTest1) {
  ...
  std::thread t7([&]() {
    ...
    EXPECT_TRUE(res);
    lock_mgr.UnlockTable(txn7, oid);
    txn_mgr.Commit(txn7);
    EXPECT_EQ(TransactionState::COMMITTED, txn7->GetState());
  });
  ...
  std::thread t8([&]() {
    ...
    EXPECT_TRUE(res);
    lock_mgr.UnlockTable(txn8, oid);
    txn_mgr.Commit(txn8);
    EXPECT_EQ(TransactionState::COMMITTED, txn8->GetState());
  });

  std::thread t9([&]() {
    bool res;
    std::this_thread::sleep_for(std::chrono::milliseconds(90));
    res = lock_mgr.LockTable(txn9, LockManager::LockMode::EXCLUSIVE, oid);
    EXPECT_TRUE(res);
    lock_mgr.UnlockTable(txn9, oid);
    txn_mgr.Commit(txn9);
    EXPECT_EQ(TransactionState::COMMITTED, txn9->GetState());
  });
...
TEST(LockManagerTest, MultTableLockUpgradeTest) {
  LockManager lock_mgr{};
  TransactionManager txn_mgr{&lock_mgr};
  table_oid_t oid = 0;
  auto *txn0 = txn_mgr.Begin();
  ...
  std::thread t1([&]() {
    ...
    res = lock_mgr.LockTable(txn1, LockManager::LockMode::INTENTION_SHARED, oid);
    EXPECT_TRUE(res);
    std::this_thread::sleep_for(std::chrono::milliseconds(1000));
    res = lock_mgr.UnlockTable(txn1, oid);
    txn_mgr.Commit(txn1);
